...
namespace TinyMayhem.Dialouge
{
    public class DialogueVertexAnimator
    {
        ...
        public IEnumerator AnimateTextIn(List<DialogueCommand> commands, string processedMessage, AudioClip voice_sound,
            Action onFinish)
        {
            textAnimating = true;
            ...
            while (true)
            {
                ...
                for (int j = 0; j < charCount; j++)
                {
                    ...
                    if
                        (charInfo
                         .isVisible) //Invisible characters have a vertexIndex of 0 because they have no vertices and so they should be ignored to avoid messing up the first character in the string whic also has a vertexIndex of 0
                    {
                        int vertexIndex = charInfo.vertexIndex;
                        int materialIndex = charInfo.materialReferenceIndex;
                        Color32[] destinationColors = textInfo.meshInfo[materialIndex].colors32;
                        Color32 theColor = j < visableCharacterIndex
        ...
        private void PlayDialogueSound(AudioClip voice_sound)
        {
            if (Time.unscaledTime - lastDialogueSound > timeUntilNextDialogueSound)
            {
                ...
                audioSourceGroup
                    .PlayFromNextSource(
                        voice_sound); //Use Multiple Audio Sources to allow playing multiple sounds at once
            }
        }
